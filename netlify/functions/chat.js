...
// ─── Retrieval function ───────────────────────────────────────────────────────
// Scores each KB chunk against the user's query using weighted term matching.
function retrieve(query, k = 4) {
  ...
  const scored = KB.map((chunk) => {
    ...
    for (const term of terms) {
      // Tag match (higher weight — tags are hand-curated keywords)
      if (tagsText.includes(term)) score += 5;
      // Content match (lower weight — broader signal)
      const hits = (contentText.match(new RegExp(term, "g")) || []).length;
      score += hits;
    }
...
exports.handler = async function (event) {
  ...
  try {
    ...
    // Find the most recent user message for retrieval
